; GIMP Script-Fu: Animation Frame Processor for shield_shimmer
; Usage: gimp -i -b '(script-fu-load "output/scripts/gimp/anim_shield_shimmer.scm")' -b '(gimp-quit 0)'
(define (process-animation-shield_shimmer output-dir frame-count)
  (let loop ((i 0))
    (when (< i frame-count)
      (let* (
      ...
      ; Apply color overlay
      (gimp-context-set-foreground '(100 180 255))
...
(process-animation-shield_shimmer "output/animations/shield_shimmer" 8)
